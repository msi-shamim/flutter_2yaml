...
class WidgetParser {
  ...
  bool _isPropertyLine(
    ...
    // Component reference tags are never properties
    if (trimmed.contains('<') && RegExp(r'<[A-Za-z]').hasMatch(trimmed)) {
      return false;
    }

    // Arrow callback: `onPressed → handleTap` (but not in comments)
  ...
  WidgetNode? _parseWidgetLine(String line) {
    ...
    // Handle component reference tags: <GroupName:Key=Value, Key=Value>
    // These are generated by the figma2flutter plugin for known components.
    // In offline mode (no MCP), output a Text placeholder with a warning.
    final compRef = RegExp(r'^<([^>]+)>\s*(?:#.*)?$').firstMatch(trimmed);
    if (compRef != null) {
      return _parseComponentRef(compRef.group(1)!, condition);
    }
  ...
  /// Parses a `<GroupName:Key=Value, Key=Value>` component reference tag.
  /// Outputs a Text widget placeholder since full component resolution
  /// requires the MCP server (figma2flutter plugin).
  WidgetNode _parseComponentRef(String refContent, String? condition) {
    // Split "GroupName:Key=Value, Key=Value" or just "GroupName"
    final colonIdx = refContent.indexOf(':');
    String groupName;
    String variantStr;
    if (colonIdx >= 0) {
      groupName = refContent.substring(0, colonIdx).trim();
      variantStr = refContent.substring(colonIdx + 1).trim();
    } else {
      groupName = refContent.trim();
      variantStr = '';
    }

    final displayText = variantStr.isNotEmpty
        ? '$groupName ($variantStr)'
        : groupName;

    // Create a Text placeholder: Text("ComponentName (Variant)")
    final node = WidgetNode(
      name: 'Text',
      condition: condition,
    );
    node.properties['text'] = '"$displayText"';

    // Store the component reference as metadata for potential future use
    node.properties['_componentRef'] = refContent;
